...
-- MRP is display-only; selling price + one shade adjustment is taxable;
-- GST is calculated per line; delivery is order-level; coupons are the only
-- additional discount.
...
begin
  ...
  loop
    ...
    v_base_price := round(coalesce(v_variant.base_price, v_variant.selling_price_override, v_product.selling_price), 2);
    v_shade_extra := case
      when coalesce(v_variant.adjustment_type, 'none') = 'none' then 0
      when coalesce(v_variant.adjustment_type, 'none') = 'percentage' then round(v_base_price * greatest(coalesce(v_variant.shade_extra_price, 0), 0) / 100, 2)
      else greatest(round(coalesce(v_variant.shade_extra_price, 0), 2), 0)
    end;
    v_unit_taxable := round(v_base_price + v_shade_extra, 2);
