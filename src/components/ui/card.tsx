...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "relative overflow-hidden rounded-[var(--radius-xl)] border border-white/70 bg-white/90",
        "shadow-[var(--shadow-sm)] backdrop-blur-sm transition-all duration-[var(--duration-base)]",
        ...
        "hover:-translate-y-0.5 hover:border-accent/20 hover:shadow-[var(--shadow-lg)]",
        className,
      )}
      {...props}
    />
...
const CardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3 ref={ref} className={cn("text-base font-bold leading-tight text-text", className)} {...props} />
  ),
);
...
const CardDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p ref={ref} className={cn("text-sm font-medium text-muted", className)} {...props} />
  ),
);
